...
## Замер: сколько времени сетка реально считает (2026-08-30)

Оценки длительности фазы 3 в этом документе считались в машинном времени.
На практике оно не равно календарному: контейнер выполнения пересоздаётся
при каждом пробуждении сессии и умирает от простоя примерно через 15 минут
после последнего действия. Сетка движется только пока он жив, поэтому интервал
пробуждений напрямую задаёт пропускную способность.

Первый замер по `scripts/duty_cycle_probe.sh` при интервале 20 минут:

| величина | значение |
|---|---|
| окно наблюдения | 25.8 мин |
| контейнер считал | 14.8 мин |
| простой | 11.0 мин |
| **занятость** | **57.3 %** |

То есть почти половина календарного времени уходила в пустоту. Интервал
пробуждений сокращён с 20 до 14 минут, чтобы следующее пробуждение приходило
примерно к моменту гибели контейнера.

Что это значит для сроков при ~72 ч машинного времени:

| занятость | календарный срок |
|---|---|
| 57 % (было) | ~5.2 суток |
| 90 % (цель) | ~3.3 суток |

Увеличение частоты чекпоинтов эту потерю не лечит: чекпоинты ограничивают
потерю **внутри** прогона при гибели контейнера (сейчас это единицы минут),
а доминирует здесь мёртвое время **между** гибелью и следующим подъёмом.
Его сокращает только частота пробуждений.

Замеры продолжают накапливаться в `reports/v2_grid/duty_cycle.log`
(не версионируется, перегенерируется).

## Второй замер: механизм пробуждений не работает (2026-09-01)

Первый замер (57% занятости) оказался слишком оптимистичным — он снят за
26 минут, внутри которых я активно работал. Замер за длинное окно даёт
принципиально другую картину.

| величина | значение |
|---|---|
| окно наблюдения | 62.3 ч |
| продвинулись на | 3 660 шагов ≈ 8.6 мин счёта |
| **занятость** | **0.23 %** |

При такой занятости 72 ч машинного времени занимают около 3.6 лет.

### Почему

Часовой страховочный Routine исправно срабатывал все 62 часа — 64 раза.
Ни одно срабатывание не стало ходом: все 64 доставились разом, скопом,
когда сессия снова стала активной. То есть Routine не поднимает контейнер,
а лишь ставит сообщение в очередь до следующей активности.

Наблюдение подтвердилось дважды: между двумя моими действиями обучение
продвигалось ровно на столько, сколько длилась моя активность
(9 150 → 12 810 → 14 640 шагов, каждый скачок — внутри рабочего хода).

**Вывод: сетка считает только пока идёт мой ход.** Ни супервизор, ни
чекпоинты, ни расписание этого не меняют — они защищают от потери
результата, но не создают машинного времени.

### Что это значит для фазы 3

Пререгистрированная сетка 32×3 = 96 прогонов ≈ 72 ч машинного времени
в этой среде недостижима. Варианты, каждый требует поправки к
пререгистрации (сокращение сетки — изменение протокола, а не деталь
исполнения):

| вариант | прогонов | машинного времени |
|---|---|---|
| полная сетка 32 конфигурации × 3 сида | 96 | ~72 ч |
| 32 конфигурации × 1 сид | 32 | ~24 ч |
| 8 конфигураций × 3 сида | 24 | ~18 ч |
| 4 конфигурации × 3 сида | 12 | ~9 ч |

Сокращать сиды опаснее, чем конфигурации: разброс между сидами на
измеренных прогонах составил 0.0016 AUPRC, и без повторов отличить
преимущество конфигурации от шума нельзя.
