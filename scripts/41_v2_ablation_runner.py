...
FINAL_DIR = ROOT / "reports" / "v2_ablations"
RESULTS = FINAL_DIR / "validation.csv"
CURVES = FINAL_DIR / "curves.csv"
PROGRESS_JSON = FINAL_DIR / "progress.json"
PROGRESS_MD = FINAL_DIR / "PROGRESS.md"
CHECKPOINTS = FINAL_DIR / "checkpoints"
...
def load_specs() -> list[V2RunSpec]:
    ...
    return [
        V2RunSpec(
            **frozen,
            ablation=ablation,
            biology_source=biology,
            aggregation=aggregation,
            seed=seed,
        )
        for ablation, biology, aggregation in variants
        for seed in range(5)
    ]
...
STRING_COLUMNS = (
    "split", "model", "ablation", "biology_source", "aggregation",
...
def main() -> int:
    ...
    specs = load_specs()
    if len(specs) != 25:
        print(f"STOP: expected 25 ablation/control runs, enumerated {len(specs)}.")
        return 1
    if len({s.run_id() for s in specs}) != 25:
        print("STOP: run_id collisions in the enumeration.")
        return 1
    if len({s.config_id() for s in specs}) != 5:
        print(f"STOP: expected 5 frozen ablation/control configs, got "
              f"{len({s.config_id() for s in specs})}.")
        return 1
    print(f"ablations: 5 frozen variants x 5 seeds = {len(specs)} runs, no collisions")
