...
RESULTS = FINAL_DIR / "v2_final_validation.csv"
CURVES = FINAL_DIR / "v2_final_validation_curves.csv"
PROGRESS_JSON = FINAL_DIR / "progress.json"
PROGRESS_MD = FINAL_DIR / "FINAL_PROGRESS.md"
...
def load_specs() -> list[V2RunSpec]:
    ...
    return [V2RunSpec(**frozen, seed=seed) for seed in range(5)]
...
def main() -> int:
    ...
    specs = load_specs()
    if len(specs) != 5:
        print(f"STOP: expected 5 final runs, enumerated {len(specs)}.")
        return 1
    if len({s.run_id() for s in specs}) != 5:
        print("STOP: run_id collisions in the enumeration.")
        return 1
    if len({s.config_id() for s in specs}) != 1:
        print(f"STOP: expected 1 frozen config, got "
              f"{len({s.config_id() for s in specs})}.")
        return 1
    print(f"final train: 1 frozen config x 5 seeds = {len(specs)} runs, no collisions")
